package studentSystem.common.infrastructure.security;
...
import studentSystem.users.application.dto.UserClaimDto;
...
public class JwtTokenProvider {
	...
	@PostConstruct
	protected void init() {
		secretKey = env.getProperty("JWT_TOKEN_SECRET_KEY");
	...
	public String buildJwtToken(UserAuthDto userAuthDto) {
		...
		jwtBuilder.setSubject(userAuthDto.getName()).setId(UUID.randomUUID().toString()).claim("isAuthenticated",
				userAuthDto.isAuthenticated());
		for (UserClaimDto userClaimDto : userAuthDto.getClaims()) {
			jwtBuilder.claim(userClaimDto.getType(), userClaimDto.getValue());
		}
		return jwtBuilder.setIssuedAt(now).setExpiration(validity).signWith(SignatureAlgorithm.HS256, this.secretKey)
				.compact();
	}
